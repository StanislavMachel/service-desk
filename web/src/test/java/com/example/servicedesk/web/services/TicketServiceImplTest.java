...
class TicketServiceImplTest {
  ...
  void getById() {
    ...
    Mockito.when(ticketRepository.findById(Mockito.any(UUID.class)))
  ...
  void getOpenTickets() {
    ...
    PageImpl<Ticket> ticketPage =
        new PageImpl<>(ticketList, PageRequest.of(0, 20), ticketList.size());

    Mockito.when(ticketRepository.findAllByStatusIn(Mockito.anyList(), Mockito.any()))
        .thenReturn(ticketPage);

    GetTicketListDto getTicketListDto =
        ticketService.getTicketsExceptWithClosedStatus(PageRequest.of(0, 20));

    Mockito.verify(ticketRepository, Mockito.times(1))
        .findAllByStatusIn(Mockito.anyList(), Mockito.any());
