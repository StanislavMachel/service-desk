...
class TicketControllerTest {
	...
	void getByIdOk() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get(TicketController.URL + "/" + id))
		...
		Mockito.verify(ticketService, Mockito.times(1)).getById(eq(id));
	}
	...
	void getOk() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get(TicketController.URL + "?page=0&size=20&sort=number,desc"))
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				...
				.andExpect(jsonPath("$.items[0].created", Matchers.is(time.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME))));
	}
	...
	void postOk() throws Exception {
		...
		Mockito.when(ticketService.create(Mockito.any())).thenReturn(new GetTicketDto()
				.setId(id)
				.setDescription(description)
				.setTitle(title)
				.setNumber(number)
		...
		mockMvc.perform(MockMvcRequestBuilders.post(TicketController.URL).contentType(MediaType.APPLICATION_JSON).content(asJsonString(postTicketDto)))
	...
	void putOk() throws Exception {
		...
		Mockito.when(ticketService.update(Mockito.any(), Mockito.any())).thenReturn(new GetTicketDto()
				.setId(id)
				.setDescription(descriptionUpdated)
				.setTitle(titleUpdated)
				.setNumber(number)
		...
		mockMvc.perform(MockMvcRequestBuilders.put(TicketController.URL + "/" + id.toString())
